...
export const listHashicorpVaultPath = async (
  ...
  const apiPath =
    data.kv_version === 2
      ? joinVaultPath(data.mount, "metadata", logicalPath)
      : joinVaultPath(data.mount, logicalPath);
...
export const getHashicorpVaultSecretMetadata = async (
  ...
  const data = await loadHashicorpConnection(projectId);
  const logicalPath = normalizeVaultPath(path);
  const secret = await readSecretData(data, logicalPath);
  return {
    path: logicalPath,
...
export const writeHashicorpVaultSecretFields = async (
  ...
  const existing = await readSecretData(data, logicalPath).catch(() => ({}));
  const next = { ...existing, ...cleanFields };
  const apiPath =
    data.kv_version === 2
      ? joinVaultPath(data.mount, "data", logicalPath)
      : joinVaultPath(data.mount, logicalPath);
  await vaultRequest(
    data,
    "POST",
  ...
  return getHashicorpVaultSecretMetadata(projectId, logicalPath);
};
